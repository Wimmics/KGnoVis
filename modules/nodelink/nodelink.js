    ...
    for (const row of result["results"]["bindings"]) {
        for (const conf of config.config) {
            ...
            for (let node of nodes) {
                ...
                if (row.hasOwnProperty(conf.target) && node.name === row[conf.target]["value"]) {
                    id_target = node.id
                }
            }
...
const buildNodes = (result, parameters, categories) => {
    ...
    for (const row of result["results"]["bindings"]) {
        for (const conf of parameters.config) {
            ...
            let submited_node_2 = {
                id: row.hasOwnProperty(conf.target) ? row[conf.target]["value"] : "",
                name: row.hasOwnProperty(conf.target) ? row[conf.target]["value"] : "",
                category: getCategory(row, conf, result["head"], categories),
                symbolSize: 25,
                itemStyle: {
                    color: color,
