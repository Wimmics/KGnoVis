...
const svg_creator = (donnees, couleurs = [0], vertical_bar = true, is_log = false, longueur = 380) => {
    ...
    const svg = d3.select("#d3_demo_test").attr("width", longueur + margin.left + margin.right).attr("height", longueur + margin.top + margin.bottom)
    ...
    group.selectAll("rect")
        ...
        .attr("x", (d,i) => origin + i*ecart + d.incr*(x_scale.bandwidth() / taille)) // Je remplace mais ça fait que j'ai le calcul * y_scale au lieu d'avoir juste y_scale
        .attr("y", (d) => y_scale(d.value))
        .attr("width", (d) => (x_scale.bandwidth() / taille) - varPadding)
